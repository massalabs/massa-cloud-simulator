...
def get_current_timestamp(genesis_timestamp_delay) -> int:
    current_date = datetime.datetime.now()
    current_ts = current_date + datetime.timedelta(seconds=genesis_timestamp_delay)
    print("Genesis Timestamp = ", current_ts)
    ...
    with open(info.env_file, "r") as f:
        ...
        for i in range(len(info.file_content)):
            if "GENESIS_TIMESTAMP=" in info.file_content[i]:
                current_ts = '"' + str(get_current_timestamp(genesis_timestamp_delay)) + '"'
...
def main(args):
    try:
        info = Info()
        genesis_ts = replace_ts_in_file(info, args.genesis_timestamp_delay)
